...
lemma dual_inter (hC : C.FG) (hC' : C'.FG) : dual p (C ∩ C') = dual p C ⊔ dual p C' := by
  have hunion : FG (dual p (↑C ∩ ↑C')) := dual_fg p (inf_fg hC hC')
  have hsup : FG (dual p ↑C ⊔ dual p ↑C') := sup_fg (dual_fg p hC) (dual_fg p hC')
  rw [← FG.dual_inj (p := p.flip) hunion hsup, dual_sup, dual_flip_dual_inter _ hC hC',
