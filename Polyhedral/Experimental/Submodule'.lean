...
variable {S R : Type*} {M : Type*}

/-- A submodule of a module is one which is closed under vector operations.
  This is a sufficient condition for the subset of vectors in the submodule
  to themselves form a module. -/
structure Submodule' (S : Type*) (R : Type*) (M : Type*)
  [Semiring S] [Semiring R] [Module S R]
...
variable [Semiring S] [Semiring R] [Module S R]
variable [AddCommMonoid M] [Module R M] [Module S M] [IsScalarTower S R M]

instance setLike : SetLike (Submodule' S R M) M where
  coe s := s.carrier
  coe_injective' p q h := by cases p; cases q; congr; exact SetLike.coe_injective' h

initialize_simps_projections Submodule' (carrier → coe, as_prefix coe)

@[simp] lemma carrier_eq_coe (s : Submodule' S R M) : s.carrier = s := rfl

/-- The actual `Submodule` obtained from an element of a `SMulMemClass` and `AddSubmonoidClass`. -/
@[simps]
def ofClass {T S R M : Type*} [Semiring S] [Semiring R] [Module S R]
    [AddCommMonoid M] [Module S M] [Module R M] [IsScalarTower S R M] [SetLike T M]
    [AddSubmonoidClass T M] [SMulMemClass T R M] (t : T) : Submodule' S R M where
  carrier := t
  add_mem' := add_mem
  zero_mem' := zero_mem _
  smul_mem' := SMulMemClass.smul_mem

instance (priority := 100) : CanLift (Set M) (Submodule' S R M) (↑)
