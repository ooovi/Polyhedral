...
-- -- alias PointedCone.IsPolyhedral := Submodule.FG

-- -- def PointedCone.IsHPolyhedral [CommRing R] [PartialOrder R] [IsOrderedRing R]
-- --     [AddCommGroup N] [Module R N] (C : PointedCone R N) : Prop :=
-- --   ∃ s : Set (Module.Dual R N), s.Finite ∧ dual .id s = C

-- -- abbrev PointedCone.IsPolyhedral [CommRing R] [PartialOrder R] [IsOrderedRing R]
-- --     [AddCommGroup N] [Module R N] (C : PointedCone R N) := C.FG

-- /-- A cone is polyhedral if it is the dual of a finite set. -/
-- def PointedCone.IsPolyhedral [CommRing R] [PartialOrder R] [IsOrderedRing R]
--     [AddCommGroup N] [Module R N] (C : PointedCone R N) : Prop :=
--   ∃ s : Set (Module.Dual R N), s.Finite ∧ dual .id s = C
...
-- instance : Coe (PolyhedralCone R M) (PointedCone R M) where
--   coe := toPointedCone
...
-- @[simp] lemma coe_toPointedCone (C : PolyhedralCone R M) : (C.toPointedCone : Set M) = C := rfl
...
--   [Module R M] [Module R N] {p : M →ₗ[R] N →ₗ[R] R} [p.IsPerfPair]
--   {s : Set M}

-- variable (p) in
-- lemma isPolyhedral_iff_exists_finite_dual_eq {C : PointedCone R N} :
...
-- variable (p) in
-- lemma IsPolyhedral_iff_exists_finset {C : PointedCone R N} :
--     IsPolyhedral C ↔ ∃ s : Finset M, dual p s = C where
...
-- variable (p) in
-- @[simp] lemma IsPolyhedral.dual_dual_flip {C : PointedCone R N} (hC : IsPolyhedral C) :
...
-- lemma isPolyhedral.dual_inj {C₁ C₂ : PointedCone R N} (hC₁ : IsPolyhedral C₁)
--   (hC₂ : IsPolyhedral C₂) : dual p.flip C₁ = dual p.flip C₂ ↔ C₁ = C₂ where
--   mp h := by rw [← hC₁.dual_dual_flip p, ← hC₂.dual_dual_flip p, h]
--   mpr h := by rw [h]

-- lemma IsPolyhedral.inf {C C' : PointedCone R M} (hC : C.IsPolyhedral) (hC' : C'.IsPolyhedral) :
--     (C ⊓ C').IsPolyhedral := by
--   obtain ⟨S, hfin, hdual ⟩ := hC
--   obtain ⟨S', hfin', hdual' ⟩ := hC'
--   use S ∪ S'
--   simp [hfin, hfin', PointedCone.dual_union, hdual, hdual']
...
--   [Module.Finite R M] [Projective R M]

-- /-- If the module `M` is finite, then the zero cone in `M` is polyhedral. -/
-- lemma IsPolyhedral.bot : IsPolyhedral (⊥ : PointedCone R M) := by
...
-- variable [AddCommGroup N] [Module 𝕜 N] -- [Module.Finite 𝕜 M] [Module.Finite 𝕜 N]
--   {p : M →ₗ[𝕜] N →ₗ[𝕜] 𝕜} {s : Set M}

-- variable (𝕜) in
-- /-- A finitely generated cone is polyhedral. -/
-- lemma IsPolyhedral.of_fg [Module.Finite 𝕜 M] (hC : C.FG) : IsPolyhedral C := by
--   classical
--   obtain ⟨s, rfl⟩ := hC
...
-- protected lemma IsPolyhedral.span [Module.Finite 𝕜 M] (hS : s.Finite) : IsPolyhedral (span 𝕜 s)
--     := .of_fg 𝕜 (fg_span hS)

-- /-- The double dual of a finite set equals the cone generated by that set. -/
-- lemma dual_dual_flip_eq_span [Module.Finite 𝕜 N] [p.IsPerfPair] {s : Set N} (hS : s.Finite) :
...
-- /-- The double dual of a finite set equals the cone generated by that set. -/
-- lemma dual_flip_dual_eq_span [p.IsPerfPair] {s : Set M} (hS : s.Finite) :
--     dual p.flip (dual p s) = span 𝕜 s := by
--   simpa using IsPolyhedral.dual_dual_flip p.flip (.span hS)
...
--   rw [← dual_span', span, ← hT, Module.Dual.eval, dual_dual_flip_eq_span T_fin]
...
-- protected lemma IsPolyhedral.dual [p.IsPerfPair] {C : PointedCone 𝕜 M} (hC : IsPolyhedral C) :
--     IsPolyhedral (dual p C) := .dual_of_fg (fg_of_isPolyhedral hC)
...
-- lemma IsPolyhedral.sup {C C' : PointedCone 𝕜 M} (hC : C.IsPolyhedral) (hC' : C'.IsPolyhedral) :
--     (C ⊔ C').IsPolyhedral := by
--   rw [PointedCone.isPolyhedral_iff_fg] at *
--   exact Submodule.FG.sup hC hC'
