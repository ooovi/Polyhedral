...
/-
# NOTE:
This file defines corank only over rings. One can generalize to semiring using the
following alternive definitions, which also naturally extend to spanCorank:

```
noncomputable def corank (S : Submodule R M) : Cardinal :=
  ⨆ T : { T : Submodule R M // Disjoint T S },
    Module.rank R T.1

noncomputable def spanCorank (S : Submodule R M) : Cardinal :=
  ⨅ T : { T : Submodule R M // Codisjoint S T },
    T.1.spanRank
```
...
theorem spanCorank_eq_spanRank_quotient :
    S.spanCorank = (⊤ : Submodule R (M ⧸ S)).spanRank
```
-/
