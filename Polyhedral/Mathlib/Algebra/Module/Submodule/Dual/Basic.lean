...
lemma dual_dualAnnihilator' (S : Submodule R M) :
  ...
  rw [← dual_dualAnnihilator, dual_eval]
...
lemma dual_eval_map (f : M₁ →ₗ[R] M₂) (C : Submodule R M₁) :
    dual (Dual.eval R M₂) (map f C) = comap f.dualMap (dual (Dual.eval R M₁) C) := by
  ext x; simp

lemma dual_eval_image (f : M₁ →ₗ[R] M₂) (s : Set M₁) :
    dual (Dual.eval R M₂) (f '' s) = comap f.dualMap (dual (Dual.eval R M₁) s) := by
  ext x; simp

lemma comap_dual_id (f : M₁ →ₗ[R] M₂) (s : Set (Dual R M₂)) :
...
theorem codisjoint_of_disjoint_dual {S T : Submodule R M}
    (hST : Codisjoint (dual p S) (dual p T)) : Disjoint S T :=
  (disjoint_dual_of_codisjoint_dual p hST).mono_right subset_dual_dual

-- # KER

lemma dual_id_range_eq_ker_flip (f : M →ₗ[R] N →ₗ[R] R) :
    dual .id f.range = ker f.flip := by
  simp [dual_dualCoannihilator, dualCoannihilator_range_eq_ker_flip]

lemma exists_fun_dual_ker (f : M →ₗ[R] N →ₗ[R] R) :
    ∃ g : N →ₗ[R] (Dual R M), dual .id g.range = ker f :=
  ⟨f.flip, dual_id_range_eq_ker_flip f.flip⟩
...
lemma dual_inf_dual_sup_dual (S T : Submodule R M) :
  ...
  change dual p ↑(S ⊓ T) = dual p S ⊔ dual p T
  simp only [dual_dualAnnihilator' p, Subspace.dualAnnihilator_inf_eq]
  exact (comap_sup_eq p.flip _ _ Fact.out).symm
...
variable (p) [Fact p.SeparatingRight] [Fact (Surjective p.flip)] in
-- NOTE: equivalently [Fact (Bijective p.flip)] (but we don't have instances for this)
