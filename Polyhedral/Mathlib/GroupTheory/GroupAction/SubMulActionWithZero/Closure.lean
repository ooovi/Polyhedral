...
This is the intersection of all `SubMulAction₀`s containing `s`. In particular,
the generated object always contains `0`, even if `0 ∉ s`. -/
def closure (s : Set M) : SubMulAction₀ R M := sInf { p | s ⊆ (p : Set M) }

theorem mem_closure :
    x ∈ closure R s ↔ ∀ p : SubMulAction₀ R M, s ⊆ p → x ∈ p := by
  simp [closure]
...
@[simp] theorem closure_eq (p : SubMulAction₀ R M) :
    closure R p = p := by
  ext x
  constructor <;> intro h
  · rw [mem_closure] at h
...
theorem closure_le {p : SubMulAction₀ R M} : closure R s ≤ p ↔ s ⊆ p := by
  constructor
  · intro h x hx
...
theorem fg_iff {p : SubMulAction₀ R M} :
    p.FG ↔ ∃ (s : Finset M), p = closure R s :=
  Set.exists_finite_iff_finset

lemma closure_gc : GaloisConnection (closure R : Set M → SubMulAction₀ R M) (↑) :=
...
variable (R) in
def smulSet (s : Set M) : SubMulAction₀ R M where
...
character `•` U+2022. -/
/-- Notation for the `SubMulAction₀` generated from a set `s`, short for
`SubMulAction₀.smulSet R s`. -/
scoped notation:70 R:70 " ∙ " s:70 => SubMulAction₀.smulSet R s
...
@[simp] theorem smulSet_eq (p : SubMulAction₀ R M) :
    R ∙ p = p := by
...
theorem smulSet_le {p : SubMulAction₀ R M} : R ∙ s ≤ p ↔ s ⊆ p := by
  constructor
...
theorem mem_smulSet_iff_forall_mem_of_subset :
    x ∈ R ∙ s ↔ ∀ p : SubMulAction₀ R M, s ⊆ p → x ∈ p := by
...
lemma smulSet_inter_left (s : SubMulAction₀ R M) (t : Set M) :
    R ∙ (s ∩ t) = s ⊓ R ∙ t := by
  rw [← smulSet_eq s]
  exact smulSet_smulSet_inter ..

lemma smulSet_inter_right (s : Set M) (t : SubMulAction₀ R M) :
    R ∙ (s ∩ t) = R ∙ s ⊓ t := by
