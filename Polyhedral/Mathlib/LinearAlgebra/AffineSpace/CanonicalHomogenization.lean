...
variable (k P) in
/-- Given an affine space `P` over `k`, `CanonicalHomogenization k P` is a vector space containing
 `P` as a hyperplane that does not pass through the origin.
...
/-- Creates an element of `CanonicalHomogenization` from a `HomogenizationExpr`. This is an
implementation detail, use `CanonicalHomogenization.ofPoint` and `CanonicalHomogenization.ofVector`
instead for constructing elements of `CanonicalHomogenization.` -/
def mk : HomogenizationExpr k V P → CanonicalHomogenization k P :=
  Quotient.mk _

private theorem mk_induction_of_point (p : P) {motive : CanonicalHomogenization k P → Prop}
...
instance : Zero (CanonicalHomogenization k P) where
  zero := mk (.ofVector 0)

instance : Add (CanonicalHomogenization k P) where
  add := Quot.map₂
    (fun
...
instance : SMul R (CanonicalHomogenization k P) where
  smul r := Quotient.map
    (fun
...
private nonrec theorem zero_smul {x : CanonicalHomogenization k P} : (0 : R) • x = 0 := by
...
private nonrec theorem add_smul {r s : R} {x : CanonicalHomogenization k P} :
    (r + s) • x = r • x + s • x := by
  obtain ⟨p⟩ : Nonempty P := inferInstance
  cases x using mk_induction_of_point p
  simp_rw [smul_mk, mk_add_mk, add_smul]

private nonrec theorem one_smul {x : CanonicalHomogenization k P} : (1 : R) • x = x := by
  obtain ⟨p⟩ : Nonempty P := inferInstance
  cases x using mk_induction_of_point p
  simp_rw [smul_mk, one_smul]

instance : AddCommGroup (CanonicalHomogenization k P) where
  zero_add x := by
    obtain ⟨p⟩ : Nonempty P := inferInstance
    cases x using mk_induction_of_point p
    refine Quot.sound <| .mk_mk ?_
...
instance : Module R (CanonicalHomogenization k P) where
...
instance : IsScalarTower R S (CanonicalHomogenization k P) where
  smul_assoc r s x := by
    obtain ⟨p⟩ : Nonempty P := inferInstance
    cases x using mk_induction_of_point p
...
/-- The embedding of the vector space into the Canonicalhomogenization. -/
def ofVector : V →ₗ[k] CanonicalHomogenization k P where
  toFun v := mk (.ofVector v)
  map_add' _ _ := rfl
  map_smul' _ _ := rfl

/-- The embedding of the affine space into the Canonicalhomogenization. -/
def ofPoint : P →ᵃ[k] CanonicalHomogenization k P where
  toFun p := mk (.mk 0 1 p)
  linear := ofVector
...
theorem induction_on {motive : CanonicalHomogenization k P → Prop} (x : CanonicalHomogenization k P)
...
theorem hom_ext {f g : CanonicalHomogenization k P1 →ₗ[k] W}
    (h : ∀ x, f (ofPoint x) = g (ofPoint x)) : f = g := by
  rwa [← LinearMap.eqLocus_eq_top, eq_top_iff, ← span_range_ofPoint, Submodule.span_le,
    Set.range_subset_iff]

theorem hom_ext_iff {f g : CanonicalHomogenization k P1 →ₗ[k] W} :
    f = g ↔ ∀ x, f (ofPoint x) = g (ofPoint x) :=
  ⟨by rintro rfl _; rfl, hom_ext⟩

/-- Auxiliary definition used for defining `CanonicalHomogenization.lift`. -/
def lift.aux (f : P →ᵃ[k] W) : CanonicalHomogenization k P → W :=
  Quotient.lift
    (fun
...
@[simp]
theorem lift_symm_linear_apply {f : CanonicalHomogenization k P →ₗ[k] W} {v : V} :
...
@[simp]
theorem lift_symm_smul {f : CanonicalHomogenization k P →ₗ[k] W} {c : R} :
...
def weight : CanonicalHomogenization k P →ₗ[k] k :=
...
theorem lift_const_apply {u : W} {x : CanonicalHomogenization k P} :
...
instance [Module.Finite k V] : Module.Finite k (CanonicalHomogenization k P) :=
