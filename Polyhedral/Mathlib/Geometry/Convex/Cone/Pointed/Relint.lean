...
/-
The relative interior (relint for short) is a topologyical notion, and hence might depend on the
...
the core iff one of the following equivalent conditions holds:
  ...
  * hull R (C ∪ (-x)) = span R C
  * ∀ t : M, ∃ ε > 0, x + ε • t ∈ C
  * ∀ φ : Dual R M, φ x = 0 → φ ∈ (dual (Dual.eval R M) C).lineal
...
true anymore. At least we always have `weak relint ≤ core`.

The core has moreover the property that the cone is the disjoint union of the cores of the faces.
This is not true for the weak relint. One still has disjointness of the relints, but not that they
cover all of the cone.
-/
...
lemma finset_sum_mem_relint_of_subset_of_le_span {s : Finset M} (hs : (s : Set M) ⊆ C)
    (hC : C ≤ Submodule.span (M := M) R s) : ∑ x ∈ s, x ∈ relint C := by
  sorry
