...
/-- A cone `S` is co-finitely generated (CoFG) precisely if its lineality space is. -/
abbrev CoFG' (C : PointedCone R M) : Prop := C.lineal.CoFG'
...
-- variable (p) [Fact p.flip.IsFaithfulPair] in -- or maybe we need `Surjective p`, not sure yet
