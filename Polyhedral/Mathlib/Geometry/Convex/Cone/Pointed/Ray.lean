...
variable {R M : Type*}
variable [Ring R] [StrongRankCondition R] [PartialOrder R] [IsOrderedRing R] [IsDomain R]
variable [AddCommGroup M] [Module R M] [Module.IsTorsionFree R M]

lemma rank_one_of_ray {x : M} (hx : x ≠ 0) : (hull R {x}).rank = 1 := by
...
lemma ray_of_rank_one (hS : C.Salient) (h : C.rank = 1) : ∃ x : M, C = hull R {x} := by
