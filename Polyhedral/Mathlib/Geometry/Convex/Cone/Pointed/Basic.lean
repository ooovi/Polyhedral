...
#check (inferInstance : HAdd (Set M) (Set M) (Set M))

protected theorem _root_.IsConvexSet.add_smul (h_conv : Convexity.IsConvexSet R s) {p q : R} (hp : 0 ≤ p) (hq : 0 ≤ q) :
    (p + q) • s = p • s + q • s := sorry
...
lemma eq_pos_smul_base {C : PointedCone R M} {f : M →ₗ[R] R} {r : R} (hf : ∀ x ∈ C, x ≠ 0 → 0 < f x) (hr : 0 < r) :
...
lemma eq_nonneg_smul_base {C : PointedCone R M} {f : M →ₗ[R] R} {r : R} (hf : ∀ x ∈ C, x ≠ 0 → 0 < f x) (hr : 0 < r)
    (hC : C ≠ ⊥) :
