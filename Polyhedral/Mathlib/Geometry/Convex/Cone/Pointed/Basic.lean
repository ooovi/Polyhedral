...
variable {R M : Type*} [Field R] [LinearOrder R] [IsOrderedRing R] [AddCommGroup M]
  [Module R M] {s : Set M} {C : PointedCone R M}

open Set LinearMap Pointwise

/-- The cone hull of a convex set is the union of the closed halflines through that set. -/
lemma mem_hull_iff_of_convex (hs : s.Nonempty) (hc : Convex R s) (x : M) :
...
theorem hull_eq_smul (hs : s.Nonempty) (hc : Convex R s) :
    hull R s = Ici (0 : R) • s := by
  ext x; exact mem_hull_iff_of_convex hs hc x

/-- Every nonzero member of the conic hull of a convex set is a positive smultiple of -/
theorem mem_hull_iff_mem_pos_smul_of_convex_nonzero {x : M} (hc : Convex R s) (hx : x ≠ 0) :
...
lemma eq_pos_smul_base {f : M →ₗ[R] R} {r : R} (hf : ∀ x ∈ C, x ≠ 0 → 0 < f x) (hr : 0 < r) :
...
lemma eq_nonneg_smul_base {f : M →ₗ[R] R} {r : R} (hf : ∀ x ∈ C, x ≠ 0 → 0 < f x) (hr : 0 < r)
    (hC : C ≠ ⊥) :
