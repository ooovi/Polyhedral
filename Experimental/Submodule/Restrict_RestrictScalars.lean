...
lemma embed_monotone {p : Submodule R M} {q₁ q₂ : Submodule S p} (hT : q₁ ≤ q₂) :
    embed q₁ ≤ embed q₂ := Submodule.map_mono hT

lemma embed_monotone_rev {p : Submodule R M} {q₁ q₂ : Submodule S p} (hq : embed q₁ ≤ embed q₂) :
    q₁ ≤ q₂ := (by simpa using @hq ·)

@[simp] lemma embed_mono_iff {p : Submodule R M} {q₁ q₂ : Submodule S p} :
    embed q₁ ≤ embed q₂ ↔ q₁ ≤ q₂ where
  mp := embed_monotone_rev
...
@[simp] lemma embed_le {p : Submodule R M} {q : Submodule S p} :
  ...
  simpa [subtype_restrictScalars] using embed_monotone (le_top : q ≤ ⊤)
...
lemma embed_disjoint_iff {p : Submodule R M} {q r : Submodule S p} :
    Disjoint q r ↔ Disjoint (embed q) (embed r) := by
  constructor
  · exact embed_disjoint
  intro hqr
  rw [disjoint_def] at ⊢ hqr
  intro x hxq hxr
  simpa using hqr x (mem_embed_iff.mpr hxq) (mem_embed_iff.mpr hxr)

lemma embed_codisjoint {p : Submodule R M} {q r : Submodule S p} (hqr : Codisjoint q r) :
    embed q ⊔ embed r = p.restrictScalars S := by
  rw [← embed_sup, codisjoint_iff.mp hqr, embed_top]
...
-- Q: Can this be expressed using LinearEquiv.ofInjective and subtype_injective?
def embed_equiv {p : Submodule R M} (q : Submodule S p) : q ≃ₗ[S] embed q where
  toFun x := ⟨q.subtype x, by simp⟩
...
def restrict_equiv (p : Submodule R M) (q : Submodule S M) :
    (p.restrictScalars S ⊓ q : Submodule S M) ≃ₗ[S] p.restrict q where
  toFun x := sorry
  invFun x := sorry
  map_add' := sorry
  map_smul' := sorry
...
def embed_orderEmbed (p : Submodule R M) : Submodule S p ↪o Submodule S M where
  toFun := embed
  inj' := embed_injective
  map_rel_iff' := embed_mono_iff

def embed_latticeHom (p : Submodule R M) :
    CompleteLatticeHom (Submodule S p) (Submodule S M) where
  toFun := embed
  map_sInf' := embed_sInf
  map_sSup' := embed_sSup

def embed_gi (p : Submodule R M) :
    GaloisCoinsertion (embed) (restrict p : Submodule S M → Submodule S p) where
  choice := fun q _ => restrict p q
